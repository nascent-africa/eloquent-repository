 ...
 *
 * @package NascentAfrica\EloquentRepository\Traits
...
trait CacheableRepository
{
    ...
    public function getCacheRepository()
    {
        if (is_null($this->cacheRepository)) {
            $this->cacheRepository = app(config('nascent-africa.repository.cache.repository', 'cache'));
        }
    ...
    public function isSkippedCache()
    {
        ...
        $skipCacheParam = config('nascent-africa.repository.cache.params.skipCache', 'skipCache');
    ...
    {
        $cacheEnabled = config('nascent-africa.repository.cache.enabled', true);

        if (!$cacheEnabled) {
            return false;
        }

        $cacheOnly = isset($this->cacheOnly) ? $this->cacheOnly : config('nascent-africa.repository.cache.allowed.only', null);
        $cacheExcept = isset($this->cacheExcept) ? $this->cacheExcept : config('nascent-africa.repository.cache.allowed.except', null);
    ...
    {
        $cacheMinutes = isset($this->cacheMinutes) ? $this->cacheMinutes : config('nascent-africa.repository.cache.minutes', 30);
